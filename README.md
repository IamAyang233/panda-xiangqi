...
| 🖌 自定义残局 | 自己摆局面（空盘起摆 · 画笔落子 · 撤销/清空）；可设执子方、目标（胜或和）与守方档位；保存到服务端全服共享，可反复挑战与删除；不评星 |
...
2. **WebSocket**：手写 RFC 6455 实现（约 300 行）替代第三方库，实现零依赖；对局历史（原计划为可选的 SQLite）改为**一局一 JSON 的棋谱目录**（见 `internal/record`），仍不引入数据库依赖；
3. **残局内容**：首批 14 关（全部引擎验证，含四大杀型教学局）；四大古谱和棋排局与 50 题扩容通过 `make puzzle-check` 内容管线持续生产；
4. **长打判负**：P0 按计划仅实现三次重复判和，长打裁决列入后续版本。

## 后续路线（§13）

在线对战（WS 架构已预留）· PWA/WASM 离线 · 更多模型协议 · 棋谱导入导出（PGN）。

棋谱记录与 AI 复盘已在 2.0.8 落地（结算弹窗「保存此局」→ 棋谱列表 → 逐手回放 →
引擎标出的关键手 → 按需问 AI 讲解），存储走「一局一 JSON + 独立目录」，与自定义残局同一套落盘原语。
