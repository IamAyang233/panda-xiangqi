...
type DiagStats struct {
	...
	// 零头按方向细分。零头里同方向的两行也能再凑成对：
	//   - 两个「加」→ 内核模式 0（add+add）
	//   - 两个「减」→ 内核模式 2（acc -= w1 + w2，2026-09-20 的 `883000d` 补上）
	// 这两个计数用来监控配对有没有真的生效：配对落地后它们应大幅下降
	// （实测单行 2.48 → 1.15 行/结点）。若某个方向又涨回来，说明新出现的
	// 条目形状没被配对逻辑覆盖。
	SingleAddRows int64
	SingleSubRows int64
	...
	// UtReplayShort 是回放时「段已用尽、拿不出条目」的次数。
	//
	// 正常情况下恒为 0：Make 的无吃子分支恰好产生两段（from 段 + to 段），
	// 而 Unmake 的 movePiece 也恰好调用 updateThreats 两次，段数严格对应。
	// 一旦非 0，说明两条路径的调用次数不再逐段配对 —— 此时回放会**静默少算条目**，
	// 评估值随之出错，而且不会有任何报错。
	//
	// 这个计数存在的唯一目的就是让那种「静默」变成可断言的事实：
	// TestThreatReplayMatchesEnumeration 会断言它恒为 0。
	UtReplayShort int64

	// FusePairsSub 是交给内核**模式 2**（acc -= w1 + w2，两个「减」配对）的次数。
	//
	// 单独记它的理由：模式 2 的循环体是后来补的，而它一旦没被走到，集成层的
	// 对拍测试（两条路径产出逐位相同）会毫无理由地通过 —— 因为那时走的其实是
	// 模式 0/1。所以它和 FusePairs 一样是「有没有被走到」的挡板，而且更精确。
	FusePairsSub int64
