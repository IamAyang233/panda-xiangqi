...
// Unmake 复用 Make 的威胁条目（翻转）vs 重新枚举。
// 两者产出的累加器逐位相同，所以这是同一份工作量的两条路径 —— 干净的 A/B。
func BenchmarkThreatReplayPairedAB(b *testing.B) {
	w, err := nnue.Load(flatPath)
	if err != nil {
		b.Skip("未找到展开后的权重，跳过")
	}
	...
	var tOff, tOn time.Duration
	for i := 0; i < b.N*5; i++ {
		nnue.SetThreatReplay(false)
		t0 := time.Now()
		round()
		tOff += time.Since(t0)

		nnue.SetThreatReplay(true)
		t0 = time.Now()
		round()
		tOn += time.Since(t0)
	}
	b.ReportMetric(float64(tOn)/float64(tOff), "replay/enum")
}

// 同方向配对（两个加 / 两个减）的边际：其余配对照常开着，只切 useSamePair。
// 两种状态都在跑「加+减」配对，所以这一对比是干净的边际而不是合计。
func BenchmarkSamePairPairedAB(b *testing.B) {
	w, err := nnue.Load(flatPath)
	if err != nil {
		b.Skip("未找到展开后的权重，跳过")
	}
	...
	var tOff, tOn time.Duration
	for i := 0; i < b.N*5; i++ {
		nnue.SetSamePair(false)
		t0 := time.Now()
		round()
		tOff += time.Since(t0)

		nnue.SetSamePair(true)
		t0 = time.Now()
		round()
		tOn += time.Since(t0)
	}
	b.ReportMetric(float64(tOn)/float64(tOff), "samepair/off")
}
